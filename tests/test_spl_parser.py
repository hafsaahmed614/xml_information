...
class TestSPLParserBasics(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.xml_dir = Path(__file__).parent.parent / 'data' / 'examples'
        print(f"DEBUG: Looking for XML files in: {cls.xml_dir}")
        print(f"DEBUG: Found {len(list(cls.xml_dir.glob('*.xml')))} files")
...
class TestRobustness(unittest.TestCase):
    ...
    def test_parse_all_sample_files_without_crash(self):
        ...
        parser = SPLParser()
        xml_dir = Path(__file__).parent.parent / 'data' / 'examples'
